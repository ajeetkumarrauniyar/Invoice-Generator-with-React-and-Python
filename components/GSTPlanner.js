...
export default function GSTPlanner() {
  ...
  // ── Step 1: B2B invoices ──
  async function runStep1(force = false) {
    if (!month.match(/^\d{6}$/)) {
      toast({
        title: "Month format galat hai",
        description: "MMYYYY format mein dalein, jaise 052026",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const res = await fetch("/api/gst/planB2B", {
        ...
        body: JSON.stringify({
          month,
          gstin: selectedCo?.gstin,
          spreadsheetId: selectedCo?.spreadsheet_id,
          planningSheet: selectedCo?.planning_sheet,
          force,
        }),
      });
      ...
      if (data.alreadyExists) {
        // Lock & Fetch — existing data return hua
        toast({
          title: `${month.slice(0, 2)}/${month.slice(2)} — existing data`,
          description: `${data.totalInvoices} invoices already hain (${data.invoiceFrom} → ${data.invoiceTo})`,
        });
        // Show regenerate option
        const shouldRegen = window.confirm(
          `ℹ️ ${month.slice(0, 2)}/${month.slice(2)} ka B2B data pehle se DB mein hai:\n` +
            `   ${data.totalInvoices} invoices (${data.invoiceFrom} → ${data.invoiceTo})\n\n` +
            `Purana data sahi hai toh Cancel dabao.\n` +
            `Naya generate karna hai (overwrite) toh OK dabao.`,
        );
        if (shouldRegen) runStep1(true);
      } else {
        toast({
          title: `B2B done — ${data.totalInvoices} invoices generated`,
          description: data.warnings?.length
            ? `${data.warnings.length} warnings — check output below`
            : "NeonDB mein save ho gaye",
        });
      }
    } catch (e) {
      setStep1Status("error");
  ...
  // ── Step 2: B2C + Exempt ──
  async function runStep2(force = false) {
    if (!month.match(/^\d{6}$/)) {
      toast({
        title: "Month format galat hai",
        description: "MMYYYY format mein dalein",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (data.alreadyExists) {
        ...
        toast({
          title: `${month.slice(0, 2)}/${month.slice(2)} — existing data`,
          description: `${data.totalInvoices} B2C/Exempt invoices already hain`,
        });
        const shouldRegen = window.confirm(
          `ℹ️ ${month.slice(0, 2)}/${month.slice(2)} ka B2C data pehle se DB mein hai:\n` +
            `   ${data.totalInvoices} invoices (${data.invoiceFrom} → ${data.invoiceTo})\n\n` +
            `Purana data sahi hai toh Cancel dabao.\n` +
            `Naya generate karna hai (overwrite) toh OK dabao.`,
        );
        if (shouldRegen) runStep2(true);
      } else {
        setStep2Range({
          from: data.invoiceFrom,
          to: data.invoiceTo,
  ...
  async function runStep3() {
    ...
    try {
      ...
      const scriptOutput = decodeURIComponent(
        res.headers.get("X-Script-Output") || "",
      );
      setStep3Status("done");
  ...
  const mmyyyy =
    month.length === 6 ? `${month.slice(0, 2)}/${month.slice(2)}` : "—";
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <StepCard number="1" title="B2B Invoices" status={step1Status}>
          <p className="text-sm text-gray-500 mb-4">
            B2B_PARTIES sheet se parties + targets padh kar invoices generate
            karta hai, Excel update karta hai, aur NeonDB mein save karta hai.
          </p>
          <Button
            onClick={() => runStep1()}
        ...
        <StepCard number="2" title="B2C + Exempt Invoices" status={step2Status}>
          <p className="text-sm text-gray-500 mb-4">
            FY2026-27 sheet se targets padhta hai (5%, 18%, exempt). CM series
            auto-detect hoti hai DB se.
          </p>
          ...
          {step2Range && (
            <p className="mt-3 text-sm text-green-700 font-medium">
              Invoice Planning:{" "}
              <span className="font-mono">{step2Range.from}</span> →{" "}
              <span className="font-mono">{step2Range.to}</span> &nbsp;·&nbsp;{" "}
              {step2Range.count} invoices
            </p>
          )}
        ...
        <StepCard number="3" title="GSTR-1 JSON" status={step3Status}>
          <p className="text-sm text-gray-500 mb-4">
            Sab sections (B2B + HSN + B2CS + EXEMP + DOCS) combine karke
            portal-ready JSON download karta hai.
          </p>
          <Button
            onClick={() => runStep3()}
          ...
          {step3Status === "done" && (
            <p className="mt-3 text-sm text-green-700 font-medium">
              ✓ <span className="font-mono">gstr1_{month}.json</span> downloaded
              — GST Portal pe upload karo
            </p>
          )}
        ...
        <StepCard number="4" title="Sales Records" status={step4Status}>
          <p className="text-sm text-gray-500 mb-4">
            NeonDB se us mahine ke saare invoices — filter, review, aur CSV
            export karo ERP entry ke liye.
          </p>
                  ...
                  {records.map((r) => (
                    <tr key={r.invoice_no} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2 text-right tabular-nums">
                        {Number(r.taxable_value).toLocaleString("en-IN", {
          ...
          {step4Status === "done" && records.length === 0 && (
            <p className="text-sm text-gray-400 mt-2">
              Is period mein koi records nahi hain.
            </p>
          )}
        </StepCard>
      </div>
